...
#pragma warning restore 0414
        ...
        private void RenderTiledMesh(Camera cam)
        {
            ...
            if (Profile.EnableReflection)
            {
                ...
                else if (cam.cameraType == CameraType.SceneView) //workaround to silent URP "Attempting to render to a depth only surface with no dummy color attachment" warning
                {
                    PMat.SetTexture(PMat.REFLECTION_TEX, Texture2D.whiteTexture);
                }
#else
                PMat.SetTexture(PMat.REFLECTION_TEX, GetReflectionRt(cam));
#endif
        ...
        {
            if (cam.cameraType == CameraType.SceneView) //workaround to silent URP "Attempting to render to a depth only surface with no dummy color attachment" warning
                return;
            ...
            if (reflectionCamera.transform.rotation != Quaternion.identity)
            {
                ...
                RenderPipeline.SubmitRenderRequest(reflectionCamera, new RenderPipeline.StandardRequest());
