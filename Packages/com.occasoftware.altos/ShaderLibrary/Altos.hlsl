...
float4 AltosClouds_float(float2 ScreenPosition)
{
...
{
    float4 clouds = AltosClouds_float(ScreenPosition);
	return InputColor.rgb * (1.0 - clouds.a) + clouds.rgb;
}


void AltosClouds_float(float2 ScreenPosition, out float3 ColorOut, out float Density)
{
    ColorOut = float3(0, 0, 0);
	Density = 1;
	#ifndef SHADERGRAPH_PREVIEW
    float4 cloud = AltosClouds_float(ScreenPosition);
